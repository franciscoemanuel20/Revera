...
export function FotosDoProduto({ productId, fotosIniciais, variantes, disponiveis }: Props) {
  ...
  // A mídia geral é a capa que o comprador vê ANTES de tocar numa cor. Dar
  // esse nome evita que ela seja cadastrada por engano dentro de uma variante
  // e a página volte para a imagem de reserva na primeira abertura.
  const grupos = [{ id: null, rotulo: "Foto de abertura do produto", corAtiva: true, abertura: true }, ...variantes.map((variante) => ({ ...variante, abertura: false }))];
  return <section className="flex flex-col gap-5 border-t border-sand pt-8">
    ...
    {grupos.map((grupo) => {
      ...
      return <article key={grupo.id ?? "geral"} className="flex flex-col gap-4 rounded-lg border border-sand p-4">
        <div><h3 className="font-semibold text-ink">{grupo.rotulo}{!grupo.corAtiva ? " (desativada)" : ""}</h3><p className="text-sm text-ink/60">{grupo.abertura ? "Esta é a imagem que o comprador vê primeiro, antes de escolher uma cor." : "A foto principal abre em destaque quando esta variante for escolhida."}</p></div>
