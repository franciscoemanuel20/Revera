-- ORDEM OBRIGATORIA
--   1) POS-VENDA.sql   (migration 8) - AINDA NAO APLICADA
--   2) este arquivo    (migration 9)
--   3) so entao publicar o codigo
-- A 9 depende das colunas que a 8 cria. Invertendo, esta falha.
-- Nao aplicar sem autorizacao: so existe UM Supabase e ele e producao.
-- Sem acento de proposito (o editor do Supabase corrompe UTF-8 pelo Chrome).
